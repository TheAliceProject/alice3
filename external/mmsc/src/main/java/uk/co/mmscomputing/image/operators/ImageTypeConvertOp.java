...
public class ImageTypeConvertOp extends Operator {
  ...
  public BufferedImage filter(BufferedImage src) {
    ...
    switch (newType) {
    case BufferedImage.TYPE_BYTE_BINARY:
      return filterByteBinary(src);
    case BufferedImage.TYPE_BYTE_INDEXED:
      return filterByteIndexed(src);
    case BufferedImage.TYPE_BYTE_GRAY:
      return filterGrayScaled(src);
    }
    return src;
  }
  ...
  private BufferedImage filterByteIndexed(BufferedImage src) {
    ...
    switch (bpp) {
    case 1:
      return new Binarization().filter(src);
    //    case 1: return new HeckbertQuantiziser(1,true,false).filter(src);
    case 2:
      return new HeckbertQuantiziser(2, true, false).filter(src);
    case 4:
      return new HeckbertQuantiziser(4, true, false).filter(src);
    case 8:
      return new DekkerQuantiziser().filter(src);
    }
    return src;
  }
  ...
  private BufferedImage filterByteBinary(BufferedImage src) {
    ...
    int threshold;
    try {
      threshold = Integer.parseInt(params.getProperty("threshold"));
    } catch (Exception e) {
      threshold = 50;
    }
    switch (bpp) {
    case 1:
      return new Binarization(threshold).filter(src);
    case 2:
      return new HeckbertQuantiziser(2, true, false).filter(src);
    case 4:
      return new HeckbertQuantiziser(4, true, false).filter(src);
  ...
  private BufferedImage filterGrayScaled(BufferedImage src) {
    ...
    switch (bpp) {
    case 8:
      return new GrayScale().filter(src);
    }
    return src;
  }

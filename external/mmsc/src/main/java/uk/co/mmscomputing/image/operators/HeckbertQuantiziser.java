...
public class HeckbertQuantiziser {
  ...
  public BufferedImage filter(BufferedImage src) {
    ...
    if (mediancut) {
      cti = 0;
      medianCut(0, 31, 0, 31, 0, 31, maxColours, w * h);
    } else {
      cti = 0;
      popularity();
    }
    ...
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        ...
        if (bitsPerPixel == 8) {
          ...
          switch (x % 4) {
          case 0:
            buf[(y * ww + x) >> 2] = (byte) (i << 6);
            break;
          case 1:
            buf[(y * ww + x) >> 2] |= (byte) (i << 4);
            break;
          case 2:
            buf[(y * ww + x) >> 2] |= (byte) (i << 2);
            break;
          case 3:
            buf[(y * ww + x) >> 2] |= (byte) i;
            break;
          ...
          switch (x % 8) {
          case 0:
            buf[(y * ww + x) >> 3] = (byte) (i << 7);
            break;
          case 1:
            buf[(y * ww + x) >> 3] |= (byte) (i << 6);
            break;
          case 2:
            buf[(y * ww + x) >> 3] |= (byte) (i << 5);
            break;
          case 3:
            buf[(y * ww + x) >> 3] |= (byte) (i << 4);
            break;
          case 4:
            buf[(y * ww + x) >> 3] |= (byte) (i << 3);
            break;
          case 5:
            buf[(y * ww + x) >> 3] |= (byte) (i << 2);
            break;
          case 6:
            buf[(y * ww + x) >> 3] |= (byte) (i << 1);
            break;
          case 7:
            buf[(y * ww + x) >> 3] |= (byte) i;
            break;
        ...
        if (dither) {                           // Dither Floyd Steinberg
          ...
          if (x < (w - 1)) {
            src.setRGB(x + 1, y, dither1(src.getRGB(x + 1, y), dr, dg, db));
          }
          if (y < (h - 1)) {
            src.setRGB(x, y + 1, dither1(src.getRGB(x, y + 1), dr, dg, db));
          }
          if ((x < (w - 1)) && (y < (h - 1))) {
            src.setRGB(x + 1, y + 1, dither2(src.getRGB(x + 1, y + 1), dr, dg, db));
          }
        }
      }
    }

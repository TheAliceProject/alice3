...
class AbstractIDE( alice.ide.IDE ):
	...
	def unsetPreviousExpression( self ):
		if self.__dict__.has_key( "_prevExpresssion" ):
			del self._prevExpresssion
	...
	def addFillIns( self, blank, type ):
		if type is alice.ast.TypeDeclaredInJava.get( org.alice.apis.moveandturn.AngleInRevolutions ):
			...
			if type is alice.ast.TypeDeclaredInJava.get( java.lang.Object ):
				...
				fillInType = type
		if self.__dict__.has_key( "_prevExpresssion" ):
			if self._prevExpresssion.getType().isAssignableTo( type ):
				blank.addChild( ecc.dennisc.alice.ide.cascade.PrevExpressionFillIn( self._prevExpresssion ) )
				blank.addSeparator()
				#self.addFillInAndPossiblyPartFills( blank, self._prevExpresssion, self._prevExpresssion.getType() )

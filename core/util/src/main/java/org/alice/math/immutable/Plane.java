...
public record Plane(double a, double b, double c, double d) {
  ...
  public Point3 getIntersection(Ray ray) {
    double t = intersect(ray);
    if (Double.isNaN(t) || (t < 0)) {
      // If t < 0 the ray is not pointing toward the plane, so they do not intersect.
      return null;
    } else {
  ...
  public double distanceTo(Point3 point) {
    double topVal = (a * point.x()) + (b * point.y()) + (c * point.z()) + d;
    double bottomVal = Math.sqrt((a * a) + (b * b) + (c * c));
    return topVal / bottomVal;
  }

  public Point3 projected(Point3 point) {
    Vector3 offsetVector = getNormal().times(-distanceTo(point));

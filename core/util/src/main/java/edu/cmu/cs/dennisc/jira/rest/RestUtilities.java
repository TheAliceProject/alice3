...
public class RestUtilities {
  ...
  private static IssueInput buildJiraIssueInput(JIRAReport jiraReport, JiraRestClient restClient) {
    final IssueInputBuilder issueBuilder = new IssueInputBuilder().setProjectKey(jiraReport.getProjectKey()).setIssueTypeId((long) jiraReport.getTypeID()).setSummary(jiraReport.getTruncatedSummary()).setDescription(jiraReport.getCreditedDescription()).setFieldValue(ENVIRONMENT_FIELD_ID, jiraReport.getEnvironment()).setFieldValue(EXCEPTION_FIELD_ID, jiraReport.getException()).setFieldValue(STEPS_FIELD_ID, jiraReport.getSteps());

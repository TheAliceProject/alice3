...
class StickyBottomLayoutManager implements java.awt.LayoutManager2 {
	private int vgap = 0;
...
public class StickyBottomPanel extends org.lgna.croquet.components.Panel {
	...
	protected java.awt.LayoutManager createLayoutManager( JPanel jPanel ) {
		final boolean IS_BORDER_PANEL = false;
		if( IS_BORDER_PANEL ) {
			return new BorderLayout();
		} else {
			return new StickyBottomLayoutManager();
		}
	}

	public void addTop( ScrollPane top ) {
		this.internalAddComponent( top, Constraint.PAGE_START.getInternal() );
	}

	public void addBottom( org.lgna.croquet.components.Component<?> bottom ) {
		this.internalAddComponent( bottom, Constraint.PAGE_END.getInternal() );
	}
}
